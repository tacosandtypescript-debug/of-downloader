...
if [[ "${PREFIX:-}" == *com.termux* ]]; then
    echo "En Termux debes usar: bash instalar-termux.sh (o deploy/termux/instalar.sh)"
    exit 1
fi
...
mkdir -p "$APP_DIR/backend" "$APP_DIR/frontend" "$APP_DIR/web"
cp -R "$SOURCE_DIR/backend/." "$APP_DIR/backend/"
cp -R "$SOURCE_DIR/frontend/." "$APP_DIR/frontend/"
chmod -R u=rwX,go= "$APP_DIR/backend" "$APP_DIR/frontend"
install -m 600 "$SOURCE_DIR/web/index.html" "$APP_DIR/web/index.html"
install -m 600 "$SOURCE_DIR/requirements/desktop.txt" "$APP_DIR/requirements-linux.txt"
...
install -m 755 "$SOURCE_DIR/deploy/linux/launcher.sh" "$BIN_DIR/of-downloader"
