...
function Get-RepoRoot {
    ...
    foreach ($candidate in $candidates) {
        if (Test-Path (Join-Path $candidate "ofbackup_cli.py")) {
...
Run-Checked $venvPython @("-m", "pip", "install", "-r", (Join-Path $repo "requirements\desktop.txt")) "No se pudieron instalar las dependencias. Si ves errores de Microsoft Visual C++, borra .venv e instala Python 3.12 o 3.11."

Step-Info "[4/5] Creando comandos..."
$ofTarget = Join-Path $binDir "of.cmd"
Set-Content -Path $ofTarget -Encoding ASCII -Value "@echo off`r`ncall `"$repo\deploy\windows\launcher.cmd`" %*`r`n"
Add-UserPath $binDir
