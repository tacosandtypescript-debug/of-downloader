...
source_has_required_files() {
    ...
    [[ -n "$candidate" \
        ...
        && -f "$candidate/deploy/termux/launcher" ]]
}
...
find_repo_root() {
    ...
    while [[ -n "$dir" && "$dir" != "/" ]]; do
        if source_has_required_files "$dir"; then
            printf '%s' "$dir"
            return 0
        fi
        dir="$(cd -- "$dir/.." && pwd)"
...
if [[ -n "${OFBACKUP_SOURCE_DIR:-}" && -d "${OFBACKUP_SOURCE_DIR}" ]]; then
    ...
    SOURCE_DIR="$(find_repo_root "${BASH_SOURCE[0]}" || true)"
elif [[ -f "$PWD/ofbackup_cli.py" ]]; then
    SOURCE_DIR="$(find_repo_root "$PWD" || printf '%s' "$PWD")"
fi
...
install_ofbackup_commands() {
    install -m 755 "$SOURCE_DIR/deploy/termux/launcher" "$PREFIX/bin/of"
    install -m 755 "$SOURCE_DIR/deploy/termux/launcher" "$PREFIX/bin/ofbackup"
